...
def observation(dataframe, count, output_path):
    for row in dataframe.iterrows():
        ...
        try:
            ...
            bodySite = None
            if row.部位代號 != row.部位名稱:  # nan != nan, 故如果自己不等於自己時，就一定是nan
            ...
            # 數值輸出成json
            observ = Observation.construct(status=status, valueQuantity=valueQuantity, category=category, performer=performer, valueString=valueString,
...
if __name__ == '__main__':
    ...
    isPATH(vital_sign_Observation_OUTPUT)

    count = 0
    print("資料處理中...")
    with open(vitalSign_path, newline='') as csv_file:
        df = pd.read_csv(csv_file, encoding='ANSI')
        observation(df, count, vital_sign_Observation_OUTPUT)
    print('Complete!')
